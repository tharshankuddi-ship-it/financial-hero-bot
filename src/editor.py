...
PEXELS_QUERIES  = ["gold coins finance", "stock market trading", "dollar bills money", "business wealth success", "bitcoin crypto gold"]
...
def _gradient_background(duration):
    ...
    for y in range(HEIGHT):
        t = y / HEIGHT
        arr[y] = [int(10 + t*-5), int(40 + t*-20), int(20 + t*-10)]
    arr[:18] = (255, 200, 0)
    arr[-18:] = (255, 200, 0)
...
def _render_caption_frame(words, highlight_index, font_path):
    ...
    start         = max(0, highlight_index - 8)
    end           = min(len(words), highlight_index + 12)
    visible_words = words[start:end]
    visible_hi    = highlight_index - start
    wrapped       = textwrap.wrap(" ".join(visible_words), width=WRAP_WIDTH)
    if not wrapped: return img
    ...
    ov_draw.rectangle([(60, y - pad), (WIDTH - 60, y + total_h + pad)], fill=(0, 0, 0, 160))
    ...
    for li, line in enumerate(wrapped):
        ...
        for wi, word in enumerate(line_words):
            ...
            color = HIGHLIGHT_COLOR if g_idx == visible_hi else CAPTION_COLOR
